...
class Customer:
    def __init__(self, x, y, counter):
        ...
        self.current_frame = 0
        self.animation_speed = 1
        self.state = "advancing"  # "retreating", "drinking", ecc.
        self.speed = 0.5
    ...
    def get_sprites(self, start_frame_count, frame_count, image):
        ...
        try:
            ...
            for i in range(start_frame_count, frame_count):
                self.rect = pg.Rect(startx + i * 35.5, starty, 35.5, 33.5)
    ...
    def move(self):
        if self.state == "advancing":
            if self.x < WINDOW_WIDTH - self.width - 20:  # si muove a destra fino al bordo
                self.x += self.speed
            else:
                print("Il cliente ha raggiunto il barista! (qui puoi gestire Game Over)")
                self.state = "waiting"
        elif self.state == "retreating":
            if self.x > 60:  # torna verso sinistra
                self.x -= self.speed
            else:
                print("Cliente tornato indietro")
                self.state = "done"


    def serve(self):
        if self.state == "advancing":
            print("Cliente servito!")
...
def main():
    ...
    customer1 = Customer(100, 175,2)
    customer2 = Customer(100, 175,2)
    customer3 = Customer(100, 175,2)
    customer4 = Customer(100, 175,2)
